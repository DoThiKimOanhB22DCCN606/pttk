...
<body>
    ...
    if(code != null) {
        ...
        if(list != null) for(Order order : list) if(order.getCode().equals(code)) {
            OrderDAO dao = new OrderDAO();
            o = dao.getOrderById(order.getId()); 
    ...
    <form action="doEditOrder.jsp" method="post">
        <table>
            <tr>
                ...
                <th>Ghi chú</th>/th><th>Phí vận chuyển</th><th>Trạng thái</th><th>Tổng tiền</th>
            </tr>
            <tr>
                <td><%=o.getCode()%></td>
        ...
        <table style="width: 60%">
            ...
            <% for(int i=0; i<o.getListFood().size(); i++) { FoodOrdered f = o.getListFood().get(i); %>
            <tr>
                <td><%=f.getFoodName()%></td>
                <td><input type="number" name="qty_food_<%=i%>" value="<%=f.getQuantity()%>" style="width:50px"></td>
                <td><button type="submit" name="action_edit_food" value="<%=i%>">Sửa</button> <button type="submit" name="action_remove_food" value="<%=i%>">Xóa</button></td>
            </tr>
            <% } %>
        </table>
        ...
        <table style="width: 60%">
            <tr><th>Tên</th><th>SL</th><th>Chọn</th></tr>
            <% for(int i=0; i<o.getListCombo().size(); i++) { ComboOrdered c = o.getListCombo().get(i); %>
            <tr>
                <td><%=c.getComboName()%></td>
                <td><input type="number" name="qty_combo_<%=i%>" value="<%=c.getQuantity()%>" style="width:50px"></td>
                <td><button type="submit" name="action_edit_combo" value="<%=i%>">Sửa</button> <button type="submit" name="action_remove_combo" value="<%=i%>">Xóa</button></td>
            </tr>
            <% } %>
        </table>
