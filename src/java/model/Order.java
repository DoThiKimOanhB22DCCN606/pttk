...
public class Order {
    ...
    // Thông tin khách hàng (JOIN từ bảng Member)
    private String customerName;
    private String customerPhone;
    private String customerAddress;
    
    // Danh sách chi tiết (để xử lý Logic Session)
    ...
    // Helper hiển thị cột "DS món ăn" trong bảng ManageOrder
    public String getFoodListAsString() {
        StringBuilder sb = new StringBuilder();
        if(listFood != null) {
            for (FoodOrdered fo : listFood) sb.append(fo.getFoodName()).append(" x").append(fo.getQuantity()).append(", ");
        }
        if(listCombo != null) {
            for (ComboOrdered co : listCombo) sb.append(co.getComboName()).append(" x").append(co.getQuantity()).append(", ");
        }
